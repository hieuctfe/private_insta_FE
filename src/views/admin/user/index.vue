    ...
    <div class="manage-question">
        <div class="card p-b-0">
            <div class="row m-b-20">
                <div class="col col-8">
                    <div class="page-title">List Users</div>
                </div>
                <div class="col col-4">
                    <div class="text-right">
                        <router-link  class="btn btn-primary" :to="{ name : 'CreateUser'}">
                            Create User
                        </router-link>
                    </div>
                </div>
            ...
            <div class="row m-b-20">
                ...
                <div class="col col-4">
                    <select name="" id="" class="form-control" v-model="pagination.role" @change="onChangeRole">
                        <option value="">All role</option>
                        <option v-for="(item, index) in roles"   :key="`level_${index}`" :value="item">{{ item | capitalize}}</option>
                    </select>
                </div>
            </div>
                    ...
                    <table class="table">
                        ...
                        <tbody v-if="pagination.data.length">
                            <tr v-for="(item , index) in pagination.data" :key="`index_${index}`">
                                ...
                                <td class="td-action">
                                    <router-link :to="{ name : 'EditUser' ,params : { id : item.id }}" class="btn btn-sm btn-outline-secondary">Edit</router-link>
                                </td>
                            </tr>
                        </tbody>
                        <tbody v-else>
...
export default {
    ...
    methods:{
        ...mapActions({
            getListEmployee : 'User/GET_LIST_EMPLOYEE',
        }),
        getList(route){
            ...
            var role = route.query.role ? route.query.role :  ''
            this.updateFilter({ page : current_page , keyword , role})
    ...
    created(){
        ...
        this.pagination.role = this.$route.query.role ? this.$route.query.role :  ''
